...
| P19 | `done` | Expat-first repackaging and relocation positioning assets | Before selling, the product must clearly read as a relocation tool for expats, not as a generic vocabulary trainer. | Product copy, pack framing, premium messaging, and onboarding language clearly target expat relocation pain points. |
| P15 | `done` | Free-tier limits and entitlement enforcement | The paywall only converts if limits are enforced consistently after payment logic exists. | Daily/plan-based limits are enforced consistently across bot, Mini App, and website. |
| P17 | `next` | Product analytics and conversion tracking | We need to see where users convert or drop before spending more effort on scaling. | Core events, funnel checkpoints, and subscription conversion metrics are implemented and documented. |
...
Notes:
- Completed initial Mini App first-run flow.
- After language selection, new users now see:
  - an onboarding intro explaining the relocation-learning loop
  - a soft premium screen with approved pricing and premium value framing
- Completion is persisted via `has_completed_onboarding`, so this is a real stateful gate and can be reused by `P14` instead of being replaced.
